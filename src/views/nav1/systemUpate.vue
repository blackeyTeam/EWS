		...
		<div class='systemUpdate'>
			<el-form ref="form" label-width="130px" size="mini">
				<el-form-item :label="$t('Firmware Version')+':'">
					<el-input v-model="firmware.version" disabled  ></el-input>
				</el-form-item>
			...
			<div class="btnBon">
				...
				<div class="Update" v-if="dispearBtn&&hasNewVer">
					<el-button type="primary" @click="pollingStatu">{{$t('Update Firmware')}}</el-button>
					<div class='versonTxt'>
						<p>{{$t('有新固件')}}:</p>
						<p>{{latestVersion}}</p>
					</div>
	...
	export default {
		data() {
			return {
				firmware: {
					version: "v1.3.0",
					new_version: "MJPMDYJ2018072",
					build_date: "2018/7/17"
				},
				hasNewVer: false,
				latestVersion: '',
				dispearBtn: false,
						...
						if (Object.keys(data).length != 0) {
							...
							console.log(11)
								data.version==data.new_version?
								 (()=>{self.strokePCT=100; self.progress = false; self.success=true;self.hasNewVer=false;})()	//更新成功
								:(()=>{ self.progress = false;self.success=false;})();
							}								
							console.log(data)
							self.firmware = data
...
<style>
	...
	.el-form-item__content{  margin-left: 10px;}
	...
	.versonTxt{color: #D5D5D5;font-size: 12px;margin-left: 10px;}
	.Update p{margin: 0;}
	.systemUpdate .el-input{width: 90%}
</style>
