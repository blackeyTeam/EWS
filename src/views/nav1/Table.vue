...
<script>
	...
	export default {
		...
		methods: {
			onScaSubmit(){
			   Object.values(this.$refs).filter(item=>{ return !item==false;})//过滤稀疏数组
			   .forEach(ele => {
				   console.log(ele)
			...
			cellBox({row, column, rowIndex, columnIndex}) {
